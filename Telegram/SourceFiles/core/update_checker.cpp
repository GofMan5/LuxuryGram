...
bool checkReadyUpdate() {
	...
	QString versionPath = readyPath + u"/tdata/version"_q;
	{
		...
		if (versionNum == 0x7FFFFFFF) { // alpha version
			...
			ClearAll();
			return false;
		// LuxuryGram: compare against LuxuryUpdateVersion, the counter the
		// packages are numbered by, not AppVersion on the upstream line.
...
void UpdateApplication() {
	if (UpdaterDisabled()) {
		const auto url = [&] {
			...
			return "https://github.com/GofMan5/LuxuryGram/releases";
