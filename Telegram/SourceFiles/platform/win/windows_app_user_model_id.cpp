...
#ifdef OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"LuxuryGram.LuxuryGramDesktop.Store";
#else // OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"LuxuryGram.LuxuryGramDesktop";
#endif // OS_WIN_STORE
...
void CleanupShortcut() {
	...
	QString path = systemShortcutPath() + u"LuxuryGram.lnk"_q;
...
bool checkInstalled(QString path = {}) {
	...
	const auto installed = u"LuxuryGram Desktop/LuxuryGram.lnk"_q;
	const auto inherited = u"AyuGram Desktop/AyuGram.lnk"_q;
	const auto old = u"AyuGram for Windows/AyuGram.lnk"_q;
	return validateShortcutAt(path + installed)
		|| validateShortcutAt(path + inherited)
