...
bool PullToNextChannel::hintVisible() const {
	// LuxuryGram: with the bottom button hidden there is no strip below the
	// scroll area to draw the hint in, and updateGeometry() never gives it a
	// rectangle -- so asking for it visible would leave it stuck shown.
	return _holding
		&& (_pull > 0.)
