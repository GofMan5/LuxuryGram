...
: HistoryItem(history, {
	...
	// LuxuryGram: upstream also turns an already-read timer media into this
	// service message. We keep the message and its media -- the burnt mark in
	// the bottom info replaces the "expired" placeholder, and a service message
	// has no bottom info to draw it in.
...
void HistoryItem::clearMediaAsExpired() {
	...
	// LuxuryGram: keeping the media means nothing here should run, but the
...
bool HistoryItem::forbidsSaving() const {
	// LuxuryGram: upstream also refuses to save extended-media invoices and
	// timer media. Lifting those restrictions is the point of the fork, so only
	// the forward ban is left here -- and forbidsForward() is neutered too.
	return forbidsForward();
}
