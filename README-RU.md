...
> [!IMPORTANT]
> Официальные сборки LuxuryGram публикуются только в разделе [Releases](https://github.com/GofMan5/LuxuryGram/releases) этого репозитория. Это портативные архивы, которые обновляются сами из этого репозитория; цифровой подписи исполняемых файлов нет, поэтому перед первым запуском сверьте контрольную сумму из `SHA256SUMS.txt`.
...
| Компонент | Статус |
| --- | --- |
| Версия продукта | [`1.0.1`](VERSIONING.md); бамп выполняется вручную или для крупного обновления |
...
С версии 1.0.1 приложение обновляется само: оно проверяет этот репозиторий на наличие новой версии и сверяет подпись обновления ключом LuxuryGram до распаковки. В версии 1.0.0 апдейтера нет, поэтому переход с неё — один разовый ручной скачок. Цифровой подписи исполняемых файлов нет, поэтому Windows SmartScreen может предупредить при первом запуске.
